...
public class ZabbixServer implements Validable {
	...
	public String getItemid(String hostid,String key){
		...
		String itemid=null;
		for(int hid=0;hid<items.get("itemid").size();++hid){
			String itemkey=DBforBIXHelper.substituteMacros(items.get("key_").get(hid),this,hostid);
			if(hostid.equals(items.get("hostid").get(hid)) && key.equals(itemkey)){
				itemid=items.get("itemid").get(hid);
				break;
			}
		}
		return itemid;
